# This code is for denoising of Salt and Pepper Noisy images
...
img_index=5 # Index of the image on which denoising is to be done (If input image name is 'Image3.png', enter 3 as img_index)

img_input = cv2.imread(r".\Data\Image"+str(img_index)+".png",0)/255  # Read the input image
noisy_img_input = cv2.imread(r".\Salt&Pepper_Noisy_Image\noisy_sp_Image"+str(img_index)+".png",0)/255 #Read the Noisy image
...
sigma_nlm=sqrt(2)*0.8 # The sigma for the NLM
...
cv2.imwrite(r".\SAP_Noise_Denoised_Result\Gauss_Filter\Gauss_Filter_Result_Image"+str(img_index)+".png",gauss_den*255) # Save the Gaussian denoised image
cv2.imwrite(r".\SAP_Noise_Denoised_Result\Non_Local_Means\NLM_Result_Image"+str(img_index)+".png",nlm_den*255) # Save the NLM denoised image
